...
struct CameraCallback:Camera::SceneCallback{
    void nextScene(const cv::Mat&mat){ //takes frame, does function
    	...
    	resize(mat, sized,cv::Size(600,300),0,0,cv::INTER_LINEAR);//resized to values that showed best results with detection parameters
    	cv::cvtColor(sized,gray,CV_BGR2GRAY); //change to grayscale
    	xs.clear(); //clear the rectangles each loop
    	cascade.detectMultiScale(gray, xs, 1.1, 0, 0, cv::Size(50, 50), cv::Size(100,100));//use detectMultiScale and get rectangles
    
    	for( std::vector<cv::Rect>::const_iterator r = xs.begin(); r != xs.end(); r++){ //loops for every rectangle with r given xs values
            	if((r->x + ((r->width)/2)) > 300){ //x coordinate for middle of rectangle further right than the middle
            		std::cout << "The x is to the right. At " << (r->x + ((r->width)/2)) << "," << (r->y + ((r->height)/2)) << ". \n";
            		}
            	else if((r->x + ((r->width)/2)) < 300){ //x coordinate for middle of rectangle further left than middle
